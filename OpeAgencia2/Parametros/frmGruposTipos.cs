...
{
    public partial class frmGruposTipos : Form
    {
        public frmGruposTipos()
        {
            InitializeComponent();
        }
        ...
        private void frmGruposTipos_Load(object sender, EventArgs e)
        {
        ...
        protected void CargarDatosIniciales()
        {
            ...
            var grupoTipos = from p in unitOfWork.GruposTiposRepository.Get()
                             select new { Id = p.GRUPO_TIPO_ID, Nombre = p.NOMBRE, Descripción = p.DESCR };
        ...
        void DeleteData()
        {
            ...
            var gruposTipos = unitOfWork.GruposTiposRepository.GetByID(iId);

            try
            {
                unitOfWork.GruposTiposRepository.Delete(gruposTipos);
                unitOfWork.Save();
        ...
        bool SalvarDatos()
        {
            ...
            BO.Models.GruposTipos oCom;


            if (usrbntMant1.bAdiciona == false)
            {
                iId = Convert.ToInt32(this.textGRUPO_TIPO_ID.Text);
                oCom = unitOfWork.GruposTiposRepository.GetByID(iId);
            }
            else
            {
                oCom = new BO.Models.GruposTipos();
            }
            ...
            {
                if (usrbntMant1.bAdiciona == false)
                    unitOfWork.GruposTiposRepository.Update(oCom);
                else
                    unitOfWork.GruposTiposRepository.Insert(oCom);
        ...
        void ConsultarDatos(int Id)
        {
            ...
            var grupoTipos = unitOfWork.GruposTiposRepository.GetByID(Id);

            MoverDatos(grupoTipos);

        }

        void MoverDatos(BO.Models.GruposTipos MyComp)
        {
